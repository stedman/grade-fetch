...
const getClasswork = require('./getClassworkByCourse.wip');
...
const harvest = async () => {
  ...
  // 2) FETCH CLASSWORK (ASSIGNMENTS) FOR EACH STUDENT
  for (let idx = 0; idx < studentKeys.length; idx += 1) {
    ...
    // b) Get classwork
    studentRecord[studentId] = await getClasswork(page);
  }
...
harvest().then((data) => {
  return saveDataToFile(data, dataDir, 'classwork-grouped.json');
});
