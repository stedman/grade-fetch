 ...
 */
const getClassworkByCourse = async (page) => {
      ...
      if (courseTableEls.length) {
        ...
        assignmentRowEls.forEach((assignRowEl) => {
          ...
          classworkData.push({
            ...
            dateAssign: extractText(tdEls[1]).trim(),
            assignment: extractText(tdEls[2]),
            category: extractText(tdEls[3]),
            score: extractText(scoreEl),
            totalPoints: extractText(tdEls[5]),
            weight: extractText(tdEls[6]),
            weightedScore: extractText(tdEls[7]),
            weightedTotalPoints: extractText(tdEls[8]),
...
module.exports = getClassworkByCourse;
